...
export class LegendaryCacheManager {
   ...
   */
  async get<T>(key: string, layers: CacheLayer[] = ['memory', 'localStorage']): Promise<T | null> {
  ...
  getMetrics(): CacheMetrics & { hitRate: number; efficiency: number } {
    ...
    const efficiency = this.metrics.avgAccessTime > 0 
   ...
   */
  private ensureSpace(neededSize: number, priority: 'low' | 'normal' | 'high'): void {
    const currentSize = this.metrics.memoryUsage;
    
    if (currentSize + neededSize <= this.MAX_MEMORY_SIZE && this.cache.size < this.MAX_ENTRIES) {
      return; // Enough space
    }
  ...
  private updateAvgAccessTime(time: number): void {
    const total = this.metrics.avgAccessTime * (this.metrics.totalOperations - 1) + time;
    this.metrics.avgAccessTime = total / this.metrics.totalOperations;
  }

  private getFromStorage(layer: 'localStorage' | 'sessionStorage', key: string): any {
    try {
  ...
  private setToStorage(layer: 'localStorage' | 'sessionStorage', key: string, data: any, expiresAt: number): void {
    try {
      const storage = layer === 'localStorage' ? localStorage : sessionStorage;
  ...
  private removeFromStorage(layer: 'localStorage' | 'sessionStorage', key: string): void {
    try {
      const storage = layer === 'localStorage' ? localStorage : sessionStorage;
      storage.removeItem(`owl_cache_${key}`);
    } catch {
  ...
  private cleanupStorage(layer: 'localStorage' | 'sessionStorage'): void {
    try {
      const storage = layer === 'localStorage' ? localStorage : sessionStorage;
      const keys = Object.keys(storage).filter(key => key.startsWith('owl_cache_'));
  ...
  private loadFromPersistentStorage(): void {
    ...
    try {
      const keys = Object.keys(localStorage).filter(key => key.startsWith('owl_cache_'));
      ...
      for (const key of keys.slice(0, 100)) { // Limit to prevent overload
        ...
        if (data) {
          this.set(realKey, data, { layers: ['memory'], ttl: this.DEFAULT_TTL });
        }
      }
    } catch {
