...
const convertUser = (user: User): ClientUser => ({
  ...
  subscription: {
    ...user.subscription,
    expiresAt: user.subscription.expiresAt ? convertTimestamp(user.subscription.expiresAt) : undefined,
  },
});
...
class ChatService {
  ...
  async createChatSession(userId: string, sessionData: CreateSessionRequest): Promise<string> {
    try {
      ...
      // Log activity
      await this.logActivity(userId, 'session_created', { sessionId: docRef.id });
  ...
  async sendMessage(
    ...
    try {
      ...
      const sessionRef = doc(db, 'users', userId, 'chatSessions', messageData.sessionId);
      await updateDoc(sessionRef, {
        lastMessage: {
          content: messageData.content,
          sender: 'user',
