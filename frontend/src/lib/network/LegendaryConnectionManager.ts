...
export class LegendaryConnectionManager {
  ...
  private async processQueue(): Promise<void> {
    while (
      ...
      this.activeConnections < this.MAX_CONCURRENT
    ) {
      const batch = this.requestQueue.splice(
        0,
        Math.min(this.BATCH_SIZE, this.MAX_CONCURRENT - this.activeConnections),
      );
  ...
  private updateCircuitBreaker(url: string, success: boolean): void {
    ...
    const allClosed = Array.from(this.circuitBreakers.values()).every(
      (cb) => cb.state === "CLOSED",
    );
  ...
  private getMaxRetries(error: unknown): number {
    ...
    if (err?.name === "AbortError") return 0; // Don't retry timeouts
    if (err?.status && err.status >= 400 && err.status < 500) return 1; // Limited retries for client errors
  ...
  getMetrics(): ConnectionMetrics & {
    ...
    const queueUtilization =
      (this.activeConnections / this.MAX_CONCURRENT) * 100;
