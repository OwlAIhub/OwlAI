 ...
 */
function validateNationalNumber(
  nationalNumber: string,
  config: CountryConfig,
): boolean {
  if (
    nationalNumber.length < config.minLength ||
    nationalNumber.length > config.maxLength
  ) {
    return false;
  }
...
export function validatePhoneNumber(phone: string): PhoneValidationResult {
  ...
  if (!validateNationalNumber(nationalNumber, countryConfig)) {
    const lengthError =
      nationalNumber.length < countryConfig.minLength
...
export function isPhoneFromCountry(
  ...
  return (
    validation.isValid &&
    validation.countryCode === COUNTRY_CONFIGS[countryCode]?.prefix
  );
}
