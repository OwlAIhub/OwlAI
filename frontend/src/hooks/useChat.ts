...
import { sendMessage, FlowiseResponse, FlowiseError } from '../lib/services/flowiseService';
...
export function useChat(options: UseChatOptions = {}): UseChatReturn {
  ...
  const handleSendMessage = useCallback(async (content: string): Promise<void> => {
    ...
    try {
      ...
      const response: FlowiseResponse = await sendMessage(content, chatId || undefined);
      ...
      const errorMessage = err instanceof FlowiseError 
        ? err.message 
  ...
  }, [isLoading, chatId, addMessage, updateMessage, onMessageSent, onMessageReceived, onError, maxRetries]);
