...
export function useRealtimeChat(options: UseRealtimeChatOptions = {}): UseRealtimeChatReturn {
  ...
  const [currentSession, setCurrentSession] = useState<ClientChatSession | null>(null);
  ...
  }, [user?.uid, currentSession, sessions, handleError]);
  ...
  const sendMessage = useCallback(async (content: string): Promise<void> => {
    ...
    try {
      ...
      const userMessageId = await chatService.sendMessage(user.uid, messageData);
      ...
      await chatService.addAIMessage(
        ...
        aiResponse.text || 'No response received',
        {
          ...
          sources: aiResponse.sourceDocuments?.map(doc => ({
            title: (doc.metadata as Record<string, unknown>)?.title as string || 'Unknown',
            url: (doc.metadata as Record<string, unknown>)?.url as string,
            relevance: (doc.metadata as Record<string, unknown>)?.score as number || 0,
          })),
        }
      );
    } catch (error) {
  ...
  }, [user?.uid, currentSession, messages, handleError, updateSession]);
  ...
  }, [user?.uid, currentSession, hasMoreMessages, messageLimit, messages, handleError]);
