...
export default function Questionnaire() {
  ...
  const handleSubjectToggle = (subject) => {
    setFormData((prevData) => {
      if (formData.curriculum === 'UGC-NET') {
        ...
        let updatedSubjects;
        if (isSelected) {
          updatedSubjects = prevData.selectedSubjects.filter((s) => s !== subject);
        } else {
  ...
  const shouldShowSubjectPopup = () => {
    if (formData.curriculum === 'UGC-NET') {
      return formData.selectedSubjects.some((s) => s !== 'Paper 1' && s !== 'Other');
    }
    return false;
  };
  ...
  return (
    <div className="flex-1 flex flex-col bg-gray-50 dark:bg-[#0D1B2A] relative">
      ...
      {/* Header */}
      <div className={`p-4 flex items-center bg-gray-50 dark:bg-[#0D1B2A] ${(showPopup || showCSIRPopup) ? 'blur-sm pointer-events-none select-none' : ''}`}>
        {currentStep > 1 && (
          <button
            onClick={handleBack}
      ...
      {/* Main Content */}
      <div className={`flex-1 flex flex-col items-center justify-center p-4 ${(showPopup || showCSIRPopup) ? 'blur-sm pointer-events-none select-none' : ''}`}>
        {/* Step 1: Name */}
        {currentStep === 1 && (
          <>
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl w-full">
              {curricula.map((item) => (
                <div
                  key={item.value}
                  onClick={() => setFormData({ ...formData, curriculum: item.value, selectedSubjects: [] })}
                  className={`cursor-pointer p-4 border rounded-xl transition shadow-sm ${
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl w-full">
              ...
              {(formData.curriculum === 'UGC-NET'
                ...
                .filter((s) => formData.curriculum !== 'UGC-NET' || s !== 'Paper 1')
                .map((subject) => (
                  <div
                    key={subject}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="bg-white dark:bg-[#1A2A3A] rounded-xl shadow-2xl p-8 max-w-md w-full text-center">
            ...
            <p className="mb-6 text-gray-700 dark:text-gray-200">
              {(() => {
                ...
                if (otherSubjects.length === 1) {
                  return (
                    <>
                      OwlAI is not yet trained for this subject you chose: <b>{otherSubjects[0]}</b>.<br />
                  ...
                  return (
                    <>
                      OwlAI is not yet trained for these subjects you chose: <b>{formatSubjectList(otherSubjects)}</b>.<br />
                      We are actively working to expand into more exams very soon.<br />
                      Stay tuned - your prep buddy is on the way!
                    </>
                  );
                  ...
                  return (
                    <>
                      OwlAI is not yet trained for the selected subject(s).<br />
                      We are actively working to expand into more exams very soon.<br />
                      Stay tuned - your prep buddy is on the way!
                    </>
                  );
            ...
            <div className="flex flex-col gap-3">
              <button
                onClick={popupLoading ? undefined : handleContinuePaper1}
                disabled={popupLoading === 'continue' || popupLoading === 'logout'}
                className={`px-6 py-2 rounded-xl font-semibold transition ${
              ...
              >
                {popupLoading === 'continue' ? 'Processing...' : 'Continue for Paper 1'}
              </button>
              <button
                onClick={popupLoading ? undefined : handleLogout}
                disabled={popupLoading === 'continue' || popupLoading === 'logout'}
                className={`px-6 py-2 rounded-xl font-semibold transition ${
                  popupLoading === 'logout'
              ...
              >
                {popupLoading === 'logout' ? 'Processing...' : 'Log out'}
              </button>
            </div>
          </div>
        </div>
      ...
      {showCSIRPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="bg-white dark:bg-[#1A2A3A] rounded-xl shadow-2xl p-8 max-w-md w-full text-center">
            ...
            <p className="mb-6 text-gray-700 dark:text-gray-200">
              Our AI is not yet trained for this exam category: <b>{formData.curriculum}</b>.<br />
              We are actively working to expand into more exams very soon.<br />
              Stay tuned - your prep buddy is on the way!
            </p>
            <div className="flex flex-col gap-3">
              <button
                onClick={popupLoading ? undefined : handleChangeExamToUGCNet}
                disabled={popupLoading === 'changeExam' || popupLoading === 'logout'}
                className={`px-6 py-2 rounded-xl font-semibold transition ${
              ...
              >
                {popupLoading === 'changeExam' ? 'Processing...' : 'Change your exam to UGC-NET'}
              </button>
              <button
                onClick={popupLoading ? undefined : handleLogout}
                disabled={popupLoading === 'changeExam' || popupLoading === 'logout'}
