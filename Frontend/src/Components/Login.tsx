...
export default function Login() {
  ...
  const [csirUserData, setCsirUserData] = useState<UserData | null>(null);
  const navigate = useNavigate();
  const colors = getColors(true); // Login is always dark mode
    ...
    <div className="min-h-screen w-full flex flex-col items-center justify-center px-4 py-8 bg-gray-50 dark:bg-[#0D1B2A]">
      <div className="max-w-md w-full space-y-8 text-center">
        ...
        {otpScreen ? (
          <>
            ...
            <button
              ...
              className={`mt-6 w-full py-2 rounded-xl font-semibold transition ${
                otpComplete && !isVerifyingOTP
                  ...
                  : `${colors.bg.primary} text-gray-800 cursor-not-allowed`
              }`}
            >
              {isVerifyingOTP
                ? "Verifying..."
            ...
            {resendTimer > 0 ? (
              ...
              <button
                ...
                className={`${colors.text.primary} ${colors.hover.textPrimary} mt-4 hover:underline ${
          ...
          <button
            onClick={handleLogin}
            className={`w-full py-2 rounded-xl font-semibold ${colors.bg.primary} text-white ${colors.hover.bgPrimary} transition`}
          >
            Continue
          </button>
        )}
        ...
        {!otpScreen && (
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-6">
            ...
            <a
              href="#"
              className={`underline ${colors.text.primary} ${colors.hover.textPrimary}`}
            >
              T&C
            </a>{" "}
            and{" "}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white dark:bg-[#1A2A3A] rounded-xl shadow-2xl p-8 max-w-md w-full text-center">
            ...
            <p className="mb-6 text-gray-700 dark:text-gray-200">
              Our AI is not yet trained for this exam category: <b>CSIR-NET</b>.
              <br />
              We are actively working to expand into more exams very soon.
              <br />
