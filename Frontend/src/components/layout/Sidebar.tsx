...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const fetchChats = useCallback(async () => {
    ...
    try {
      const response = await api.session.getChatSessions(studentData.uid);
      
      if (response.status === "success" && response.data) {
        const formattedChats: ChatSession[] = response.data.map(session => ({
  ...
  return (
    <div 
      ...
      } transition-all duration-300 ease-in-out w-64 lg:w-72 border-r ${
